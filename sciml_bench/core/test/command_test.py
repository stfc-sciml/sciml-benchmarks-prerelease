...
def test_command_run_single_benchmark(tmpdir, mocker):
    mocker.patch('sciml_bench.core.command.run_benchmark')
    runner = CliRunner()
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            cli, ['run', '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])
...
def test_command_run_single_benchmark_set_verbosity(tmpdir, mocker, caplog):
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            cli, ['run', '--verbosity=0', '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            cli, ['run', '--verbosity=2', '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])
        assert result.exit_code == 0
        assert os.environ['TF_CPP_MIN_LOG_LEVEL'] != '-1'
        ...
        result = runner.invoke(cli, ['run', '--verbosity=3', '--log-level=error',
                                     '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])
        assert result.exit_code == 0
        assert os.environ['TF_CPP_MIN_LOG_LEVEL'] >= '-1'
...
def test_command_run_single_benchmark_set_environment(tmpdir, mocker, caplog):
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(
            cli, ['run', '--use-amp', '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])
        assert result.exit_code == 0
        assert os.environ['TF_ENABLE_AUTO_MIXED_PRECISION'] == '1'

        # Disable GPU and use CPU only
        result = runner.invoke(
            cli, ['run', '--cpu-only', '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])
        assert result.exit_code == 0
...
def test_command_run_single_benchmark_fails_invalid_data_dir(tmpdir, mocker, caplog):
    ...
    with runner.isolated_filesystem():
        ...
        assert 'Data directory {} does not exist!'.format(
            data_dir) in caplog.text


def test_command_run_single_benchmark_fails_invalid_benchmark_name(tmpdir, mocker, caplog):
    mocker.patch('sciml_bench.core.command.run_benchmark')
    runner = CliRunner()
    with runner.isolated_filesystem():
        model_dir = str(tmpdir)
        ...
        # Run command
        result = runner.invoke(
            cli, ['run', '--data-dir', data_dir, '--model-dir', model_dir, 'random-benchmark'])

        assert result.exit_code == 1
        assert 'No benchmark with name ' in caplog.text
...
def test_command_run_single_benchmark_skips_invalid_data_dir_for_specific_benchmark(tmpdir, mocker, caplog):
    ...
    with runner.isolated_filesystem():
        ...
        assert result.exit_code == 0
        assert 'Data directory {} does not exist!'.format(
            data_dir + '/em_denoise') in caplog.text
        assert 'Skipping benchmark em_denoise'.format(data_dir) in caplog.text
...
def test_command_run_single_benchmark_fails_invalid_data_dir_for_specific_benchmark(tmpdir, mocker, caplog):
    ...
    with runner.isolated_filesystem():
        ...
        # Run command
        result = runner.invoke(
            cli, ['run', '--no-skip', '--data-dir', data_dir, '--model-dir', model_dir, 'em_denoise'])

        assert result.exit_code == 1
        assert 'Data directory {} does not exist!'.format(
            data_dir + '/em_denoise') in caplog.text
        assert 'Skipping benchmark em_denoise'.format(
            data_dir) not in caplog.text
...
def test_command_run_all_benchmarks(tmpdir, mocker, caplog):
    ...
    with runner.isolated_filesystem():
        ...
        # Run command
        result = runner.invoke(
            cli, ['run', '--data-dir', data_dir, '--model-dir', model_dir])
